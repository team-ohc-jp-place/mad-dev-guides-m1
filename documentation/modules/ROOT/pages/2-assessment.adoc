...
関連づけられているのでアプリケーションは Summer Framework のアーキタイプからアーキタイプタグを継承しています。 *Application Profile* から `Tags` タブをクリックし、`Archetype` でフィルタリングしてください。 _Summer Framework_ と _Tomcat_ のタグが表示されます。
