...
**Migration Waves management** ビジネス価値を提供するワークロードの小さな集合体です。 MTA の移行ウェーブは、指定したスケジュールで移行するアプリケーションをグループ化します。さらに、マイグレーションウェーブでは、ウェーブのアプリケーションのリストを Jira 課題管理システムにエクスポートすることができます。これにより、移行ウェーブの各アプリケーションに対して、追跡用の個別の Jira 課題が自動的に作成されます。
